...
const constructConfig = (config) => {
   ...
   config.ttlInMs.freeUnit =
      config.ttlInMs.freeUnit || CONFIG.ttlInMs.freeUnit
...
class WeCashedApi {
   constructor(config = CONFIG) {
      ...
      this.cacheProvider = new config.CashProvider(this.ttlInMs)
   ...
   async #executeApiCall(key, apiCall, ...args) {
      const data = this.cacheProvider.get(key)
